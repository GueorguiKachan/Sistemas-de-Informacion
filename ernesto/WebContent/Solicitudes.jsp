...
<%@ page import="es.unizar.sisinf.grp1.model.SolicitudFacade" %>
...
<div class="container-fluid">
	...
	<div class="table-responsive">
		<table class="table table-striped table-hover mx-auto w-auto">
			<tr>
			<th colspan="5" style="text-align:center">Solicitudes de cambio</th></tr>
			<tr>
			<th>Jugador<img src="" alt="" style="float:right;"></img></th>
				<th>Equipo<img src="" alt="" style="float:right;"></th>
				<th>Campo a cambiar<img src="" alt="" style="float:right;"></th>
				<th>Nuevo Valor<img src="" alt="" style="float:right;"></th>
				<th>Usuario<img src="" alt="" style="float:right;"></th>
			</tr>
			<tbody>
	<%
	SolicitudFacade daosolicitud  = new SolicitudFacade();
	List<SolicitudVO> lista = daosolicitud.getTodasSolicitudes();
	EquipoFacade dao2= new EquipoFacade();
	JugadorFacade jugadordao = new JugadorFacade();
	JugadorVO jugadorvo= null;
	for(SolicitudVO solicitud : lista)
	{
		if (solicitud.getClass().getSimpleName().equals("SolicitudJugadorVO")){
			jugadorvo = jugadordao.getPlayer(solicitud.getCodJugador());
			
		
		%>
	    <tr>
	    <form action="editarJugador" method="get">
	    <input type="hidden" name="id" value="<%=solicitud.getId()%>"/>
	      <td><%=jugadorvo.getNombre()%></td>
	      <td><%=jugadorvo.getEquipo()%></td>
	      <td><%=solicitud.getCampo()%></td>
	      <td><%=solicitud.getValor()%></td>
	      <td><%=solicitud.getNomUser()%></td>      
	      <td><button type="submit" class="fas fa-edit">Editar</button></td>
	      <td><a href='borrarSolicitudJugador?id=<%=solicitud.getId()%>' class="fas fa-edit button">Eliminar</a></td>
	     </form>                       
	     </tr>                        
	      ...
	      <% }else{%>
	      <tr>
	    <form action="editarEquipo" method="get">
	     <input type="hidden" name="id" value="<%=solicitud.getId()%>"/>
	      <td>-</td>
	      <td><%=solicitud.getCodEquipo()%></td>
	      <td><%=solicitud.getCampo()%></td>
	      <td><%=solicitud.getValor()%></td>
	      <td><%=solicitud.getNomUser()%></td>      
	      <td><button type="submit" class="fas fa-edit">Editar</button></td>
	      <td><a href='borrarSolicitudEquipo?id=<%=solicitud.getId()%>' class="fas fa-edit button">Eliminar</a></td>
	     </form>                       
	     </tr> 
	      <% }}%>     
